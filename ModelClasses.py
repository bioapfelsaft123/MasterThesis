...
class DayAheadProblem():
    ...
    def _results(self):
        ...
        self.res.CapturePrices.loc['Average_DA_Price'] = self.res.DA_Prices.mean(axis=0).values
         ...
         # ---- Calculate Average Total Generation (per technology) ----
        # EGen shape: (N_Cap, N_Hours, N_Scen)
        total_gen_per_scenario = self.res.EGen.sum(axis=1)  # Shape: (N_Cap, N_Scen)
        average_total_gen = total_gen_per_scenario.mean(axis=1)  # Shape: (N_Cap,)

        # Create DataFrame with Technology names
        self.res.AverageTotalGeneration = pd.DataFrame({
            'Technology': self.D.TechInfo['Technology'].values,
            'Average_Annual_Generation_MWh': average_total_gen
